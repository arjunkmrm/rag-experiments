...
class SafeOpenai:
    def __init__(self, keys=None, start_id=None, proxy=None):
        ...
        client.proxy = proxy
        client.api_key = self.key[self.key_id % len(self.key)]
    ...
    def chat(self, *args, return_text=False, reduce_length=False, **kwargs):
        while True:
            try:
                ...
                if "gpt" in model:
                    completion = client.chat.completions.create(*args, **kwargs, timeout=30)
                ...
                client.api_key = self.key[self.key_id]
                time.sleep(0.1)
        if return_text:
    ...
    def text(self, *args, return_text=False, reduce_length=False, **kwargs):
        while True:
            try:
                completion = client.Completion.create(*args, **kwargs)
                break
            except Exception as e:
                print(e)
                if "This model's maximum context length is" in str(e):
